<template>
  <div class="w-full max-w-xl mx-auto">
    <div class="bg-gradient-to-b from-gray-50 to-gray-100 shadow-2xl rounded-lg border-t-4 border-[#D4AF37] ">
      ...
      <!-- Contenedor principal con animaciones CSS -->
      <div>
        <!-- Mensaje de éxito (mostrado solo cuando registerSuccess es true) -->
        <div v-if="registerSuccess" class="p-8 flex flex-col items-center justify-center space-y-6 animate-fadeIn">
        ...
        <!-- Formulario (oculto cuando registerSuccess es true) -->
        <form v-else @submit.prevent="registrarUsuari" class="p-8 space-y-6">
      ...
      <!-- Secció d'inici de sessió (solo visible cuando no hay éxito de registro) -->
      <div v-if="!registerSuccess" class="py-6 px-8 bg-gradient-to-r from-[#800040] to-[#f1948a] border-t-2 border-[#800040]">
...
async function registrarUsuari() {
  ...
  try {
    ...
    const response = await fetch('http://cinema.daw.inspedralbes.cat/api/register', {
      method: 'POST',
      headers: {
    ...
    if (data.token) {
      ...
      if (data.user) {
        ...
        try {
          ...
          const userResponse = await fetch('http://cinema.daw.inspedralbes.cat/api/user', {
            headers: {
